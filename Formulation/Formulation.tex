...
\begin{keyword}
Resource Allocation \sep Mixed-Integer Programming \sep Stochastic Optimization \sep Multiple Knapsack Problem \sep Column Generation \sep Benders Decomposition \sep Decision Support Systems
\end{keyword}
...
\midrule
\endfirsthead
\caption[]{Complete Notation Reference (continued)} \\
\toprule
Symbol & Description \\
\midrule
\endhead
...
\midrule
\multicolumn{2}{l}{\textit{Stochastic Return Parameters}} \\
$\tilde{\alpha}_{is}^d$ & Stochastic return for task $i$, scenario $s$, dimension $d$ \\
\midrule
\multicolumn{2}{l}{\textit{Multi-Agent Extension}} \\
...
\subsection{Stochastic Returns Extension}

The formulation above models uncertainty solely through stochastic recovery requirements $\beta_s$. However, in practice, the returns from task engagement are also uncertain: productivity may vary due to cognitive fluctuations, environmental interruptions, and task complexity variations. We extend the model by introducing scenario-dependent return coefficients.

Let $\tilde{\alpha}_{is}^d$ denote the return rate for task $i$ in dimension $d \in \{I, Y, K, O\}$ under scenario $s$. The extended utility function becomes:
\begin{align}
...
\subsection{Structural Complexity and Non-Triviality}
...
\begin{proposition}[Non-Triviality]
The optimal allocation problem cannot be reduced to a greedy ranking of tasks by weight, even when all parameters are deterministic.
\end{proposition}

\textit{Proof sketch.} The greedy approach fails due to four structural features:

\begin{enumerate}
    \item \textbf{State-dependent coupling across periods.} The priority weight $w_I(t, g_t)$ depends on $g_t$, which in turn depends on the cumulative allocation $\sum_{\tau=1}^{t} h_{1,\tau}$. Allocating more hours to Task 1 today reduces its future weight through the $(1 - g_t)$ factor, creating an intertemporal trade-off that cannot be resolved by single-period greedy ranking.
    
    \item \textbf{Multiple resource constraints.} Each period involves three simultaneous capacity constraints (time, energy, emotional load), creating a multi-dimensional knapsack structure. Multi-dimensional knapsack problems are NP-hard, and greedy algorithms do not guarantee optimality even in the single-period case.
    
    \item \textbf{Minimum allocation requirements.} The logical linking constraints $h_i^{\min} \cdot x_{it} \leq h_{it}$ create discrete jumps in feasible allocations. A task cannot receive a fractional amount below its minimum, inducing combinatorial structure that precludes continuous greedy solutions.
    
    \item \textbf{Stochastic recovery coupling.} The recovery constraints link emotional load (a function of $h_{it}$) to available time through scenario-dependent recovery hours $r_{ts}$. This creates cross-constraint dependencies that a greedy approach cannot account for.
\end{enumerate}
...
\textbf{Extended notation.} Let $\mathcal{J} = \{1, 2, \ldots, |\mathcal{J}|\}$ denote the set of agents. We introduce agent-indexed decision variables:
\begin{itemize}
    \item $x_{ijt} \in \{0,1\}$: binary indicator, 1 if agent $j$ undertakes task $i$ on day $t$
    \item $h_{ijt} \in \R_+$: hours allocated by agent $j$ to task $i$ on day $t$
    \item $r_{jts} \in \R_+$: recovery hours for agent $j$ on day $t$ under scenario $s$
    \item $z_{jts} \in \{0,1\}$: high emotional load indicator for agent $j$ on day $t$, scenario $s$
\end{itemize}

\textbf{Multi-agent objective.} The organization maximizes total expected discounted utility across all agents:
\begin{align}
...
\textbf{Agent-specific constraints.} Each agent faces individual resource limits:
\begin{align}
\sum_{i \in \mathcal{I}} h_{ijt} + r_{jts} &\leq H_j^{\max} \quad \forall j \in \mathcal{J}, t \in \mathcal{T}, s \in \mathcal{S} \\
\sum_{i \in \mathcal{I}} e_i \cdot h_{ijt} &\leq E_j^{\max} \quad \forall j \in \mathcal{J}, t \in \mathcal{T} \\
\sum_{i \in \mathcal{I}} m_i \cdot h_{ijt} &\leq M_j^{\max} \quad \forall j \in \mathcal{J}, t \in \mathcal{T}
\end{align}

\textbf{Coupling constraints.} Shared organizational resources introduce coupling across agents:
\begin{align}
...
\subsection{Advanced Solution Algorithms}\label{sec:algorithms}
