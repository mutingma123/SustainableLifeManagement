...
\begin{frame}{Full Formulation}
	...
	\onslide<3->{%
	\vspace{-0.35cm}
	\begin{equation}\tag{Assign}\label{eq:assign}
	...
	\onslide<5->{%
	\vspace{-0.35cm}
	\begin{equation}\tag{Chance}\label{eq:chance}
		\sum_{\omega \in \Omega} p_\omega\, y_j^\omega \leq \alpha,
		\quad \forall j
	\end{equation}}
	\onslide<6->{%
...
\begin{frame}{Master Problem (Iteration $t$)}
	...
	\onslide<1->{%
	\begin{equation}\tag{MP-Obj}\label{eq:mp-obj}
		\max_{x,U,Z} \quad
...
\begin{frame}{Feasibility Cut Generation: No-Good Cut}
	...
	\begin{itemize}
		\item<1-> Let $\mathcal{I}_j(\bar{x}) = \{i \in \mathcal{I} : \bar{x}_{ij} = 1\}$
			be the \textbf{full bundle} of tasks assigned to agent $j$.
		\item<2-> The simplest valid cut forbids reassigning the exact same bundle:
			\[
				\sum_{i \in \mathcal{I}_j(\bar{x})} x_{ij}
				\;\leq\; |\mathcal{I}_j(\bar{x})| - 1.
				\quad \text{(No-good cut)}
			\]
		\item<3-> \textbf{Guarantee}: eliminates the current infeasible assignment
			pattern, so the algorithm never revisits it.
		\item<4-> \textbf{Limitation}: only excludes the \emph{entire} bundle.
			Other bundles that share the same offending tasks remain feasible
			to the master, so convergence can be slow.
	\end{itemize}
	\onslide<5->{%
	\vspace{0.1cm}
	\small The no-good cut ensures finite convergence but may require
	many iterations because it eliminates only one assignment at a time.}
\end{frame}

\begin{frame}[shrink=5]{Feasibility Cut Generation: Stronger Cut}
	The stronger cut targets the \textbf{root cause} of the violation.
	\begin{itemize}
		\item<1-> Within the full bundle $\mathcal{I}_j(\bar{x})$, identify a
			\textbf{minimal offending subset}
			$\mathcal{B}_j(\bar{x}) \subseteq \mathcal{I}_j(\bar{x})$ such that
			\[
				\sum_{\omega \in \Omega:\;
				\sum_{i \in \mathcal{B}_j(\bar{x})} S_{ij}^\omega > C_j}
				p_\omega \;>\; \alpha.
			\]
		\item<2-> This subset alone already causes the chance violation.
			Add the cut:
			\[
				\sum_{i \in \mathcal{B}_j(\bar{x})} x_{ij}
				\;\leq\; |\mathcal{B}_j(\bar{x})| - 1.
				\quad \text{(Stronger cut)}
			\]
		\item<3-> Because $\mathcal{B}_j \subseteq \mathcal{I}_j$
			and $|\mathcal{B}_j| \leq |\mathcal{I}_j|$, the stronger cut
			\textbf{dominates} the no-good cut: it eliminates
			\emph{every} assignment that contains the offending subset,
			not just the one specific bundle.
	\end{itemize}
	\onslide<4->{%
	\vspace{0.1cm}
	\begin{block}{Practical effect}
		Fewer iterations are needed because each stronger cut removes a
		larger region of infeasible assignments from the master problem.
	\end{block}}
\end{frame}

\begin{frame}[shrink=5]{No-Good vs.\ Stronger Cut: Comparison}
	\onslide<1->{%
	\textbf{Example.} Agent $j$ is assigned tasks $\{1,2,3\}$ and violates the
	chance constraint. Suppose tasks $\{1,3\}$ alone already cause violation.}
	\begin{itemize}\small
		\item<2-> \textbf{No-good cut} forbids $\{1,2,3\}$ together:
			$x_{1j} + x_{2j} + x_{3j} \leq 2$. \\
			The master can still assign $\{1,3\}$ to $j$, which is also
			infeasible, requiring another iteration.
		\item<3-> \textbf{Stronger cut} forbids $\{1,3\}$ together:
			$x_{1j} + x_{3j} \leq 1$. \\
			This also eliminates $\{1,2,3\}$, $\{1,3,4\}$, and any other
			bundle containing the offending pair.
	\end{itemize}
	\onslide<4->{%
	\begin{center}
	\small
...
\begin{frame}{Algorithm: Step-by-Step}
	\begin{enumerate}
		...
		\item<5-> \textbf{If infeasible}: for each violating agent $j$,
			generate a \textbf{stronger cut} (or a no-good cut if no smaller
			offending subset is found) and add it to $\mathcal{B}^{t+1}$.
		\item<6-> \textbf{Update.} Set $t \leftarrow t + 1$. Return to Step~2.
	\end{enumerate}
	\onslide<7->{%
	\vspace{0.2cm}
	\begin{block}{Convergence}
		Both cut types guarantee finite termination: each cut eliminates
		at least one infeasible assignment pattern, and the total number
		of binary patterns is finite. Stronger cuts accelerate convergence
		by eliminating multiple patterns per iteration.
	\end{block}}
\end{frame}
...
\begin{frame}{Why This Decomposition Works}
	\begin{itemize}
		...
		\item<3-> \textbf{Managerial interpretation}: the planner proposes an
