...
\begin{frame}{Full Formulation}
	...
	\[\sum_{i} S_{ij}^\omega\, x_{ij}
		\leq C_j + M\, y_j^\omega,
		\quad \forall j,\omega \quad \text{(Cap)}
	\]}
	\onslide<5->{%
	\vspace{-0.5cm}
	\[\sum_{\omega} p_\omega\, y_j^\omega \leq \alpha,
		\quad \forall j \quad \text{(Chance)}\label{eq:chance}
	\]}
	\onslide<6->{%
	\vspace{-0.5cm}
	\[U_j = \tfrac{1}{C_j}\textstyle\sum_{i}\sum_{\omega}
		p_\omega S_{ij}^\omega x_{ij},
...
\begin{frame}{General Benders Principle (from Lecture)}
	\onslide<1->{%
	A large program $[P]$ with variables partitioned into groups $x$ and $y$:
	\[
		[P] \quad \min \; c'x + d'y
		\quad \text{s.t.} \quad Ax + By = e, \;\; x \geq 0, \;\; y \in Y.
	\]}
	\onslide<2->{%
	\textbf{Step 1.} Fix $y$ and solve the \emph{primal Benders subproblem} (SPP):
	\[
		h(y) = \min\{c'x : Ax = e - By,\; x \geq 0\}.
	\]}
	\onslide<3->{%
	\textbf{Step 2.} Derive the \emph{dual subproblem} (SPD):
	\[
		h(y) = \max\{u'(e - By) : u'A \leq c\}.
	\]}
	\onslide<4->{%
	\textbf{Step 3.} Reformulate $[P]$ as a \emph{master problem} with
	\textbf{optimality cuts} from extreme points and
	\textbf{feasibility cuts} from extreme rays of the dual domain.}
\end{frame}

\begin{frame}{General Benders: Master Problem and Cuts}
	\onslide<1->{%
	The equivalent master problem is:
	\[
		\min \; z + d'y
	\]}
	\onslide<2->{%
	subject to \textbf{optimality cuts} (from extreme points $\omega_p$, $p \in \Omega$):
	\[
		z - \omega_p'(e - By) \geq 0, \quad \forall p \in \Omega
	\]}
	\onslide<3->{%
	and \textbf{feasibility cuts} (from extreme rays $\rho_r$, $r \in \Gamma$):
	\[
		\rho_r'(e - By) \leq 0, \quad \forall r \in \Gamma
	\]}
	\onslide<4->{%
	\begin{block}{Constraint Generation}
		Instead of enumerating all cuts, we solve a relaxed master and add
		violated cuts iteratively. The algorithm converges in finitely many
		iterations because the number of extreme points and rays is finite.
	\end{block}}
\end{frame}
