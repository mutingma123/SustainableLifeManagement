...
def _AddLogicalConstraints(Opt):
    for Task in Opt.Tasks:
        for Day in range(config.HORIZON):
            Opt.Model.addConstr(
                Opt.H[Task.Name, Day] >= Task.MinHours * Opt.X[Task.Name, Day]
            )
            Opt.Model.addConstr(
                Opt.H[Task.Name, Day] <= Task.MaxHours * Opt.X[Task.Name, Day]
            )
...
def _AddRecoveryConstraints(Opt):
    for Day in range(config.HORIZON):
        EmotionSum = quicksum(
            T.Emotion * Opt.H[T.Name, Day] for T in Opt.Tasks if T.Emotion > 0
        )
        for Scen in range(config.SCENARIOS):
            Opt.Model.addConstr(
                EmotionSum >= 0.8 * config.MAX_EMOTION - 1000 * (1 - Opt.Z[Day, Scen])
            )
            Opt.Model.addConstr(
                Opt.R[Day, Scen] >= config.RECOVERY_HOURS[Scen] * Opt.Z[Day, Scen]
            )


def _AddMinimumRequirements(Opt):
    # Weekly Task1 minimum (high-priority task)
    for Week in range(config.HORIZON // 7 + 1):
        DayStart = Week * 7
        DayEnd = min((Week + 1) * 7, config.HORIZON)
        Task1Sum = quicksum(Opt.H["Task1", Day] for Day in range(DayStart, DayEnd))
        Opt.Model.addConstr(Task1Sum >= 10)
